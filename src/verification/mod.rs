...
use crate::optimizer::{CmpOp, Instr, Op}; // Make sure Instr, Op, CmpOp are imported
...
impl<'a> Vm<'a> {
    ...
    fn get_var(&self, name: &str) -> i64 {
        *self.variables.get(name).unwrap_or_else(|| panic!("Variable '{}' used before definition", name))
    }
            ...
            let cycles = match instr {
                ...
                Instr::BinOp(dest, left, op, right) => {
                    ...
                    let result = match op {
                        ...
                        Op::Mul => lval * rval,
                        Op::Div => {
                            if rval == 0 { panic!("Division by zero"); }
                            lval / rval
                        }
                ...
                Instr::BranchIf(cond, then_label, else_label) => {
                    ...
                    let target_label = if cond_val != 0 { then_label } else { else_label };
                    next_pc = *self.labels.get(target_label).unwrap_or_else(|| panic!("Branch target label '{}' not found", target_label));
                    3 // Cycle cost (branch)
                }
                Instr::Jump(label) => {
                    next_pc = *self.labels.get(label).unwrap_or_else(|| panic!("Jump target label '{}' not found", label));
                    2 // Cycle cost (jump)
                }
...
mod tests {
    ...
    fn test_calculate_cycles_basic() {
        let instrs = vec![
            ...
            Instr::BinOp("t3".to_string(), "t1".to_string(), Op::Add, "t2".to_string()), // pc=3, cycles=2+1=3, t3=30
    ...
    fn test_calculate_cycles_branch_taken() {
         let instrs = vec![
            ...
            Instr::Cmp("cmp_res".to_string(), "cond".to_string(), CmpOp::Eq, "one".to_string()), // pc=3, cycles=2+1=3, cmp_res=1 (true)
            Instr::BranchIf("cmp_res".to_string(), "then".to_string(), "else".to_string()), // pc=4, cycles=3+3=6, branches to "then" (pc=5)
    ...
    fn test_calculate_cycles_branch_not_taken() {
         let instrs = vec![
            ...
            Instr::Cmp("cmp_res".to_string(), "cond".to_string(), CmpOp::Eq, "one".to_string()), // pc=3, cycles=2+1=3, cmp_res=0 (false)
            Instr::BranchIf("cmp_res".to_string(), "then".to_string(), "else".to_string()), // pc=4, cycles=3+3=6, branches to "else" (pc=8)
    ...
    #[should_panic(expected = "Branch target label 'missing_else' not found")]
     fn test_panic_on_missing_branch_label() {
         let instrs = vec![
            ...
            Instr::BranchIf("cond".to_string(), "start".to_string(), "missing_else".to_string()),
         ];
         calculate_cycles(&instrs);
     }
     ...
     fn test_calculate_cycles_with_phi() {
         let instrs = vec![
            ...
            Instr::Phi("c".to_string(), vec![("entry".to_string(), "a".to_string())]), // pc=5, cycles=3+1=4, c=5 (from a)
