...
use crate::optimizer::{CFG, BasicBlock, Instr, Op, CmpOp};
...
pub fn create_test_cfg(blocks_data: Vec<(&str, Vec<Instr>, Vec<&str>, Vec<&str>)>) -> CFG {
     ...
     //     }
     // }
     CFG { blocks, current_block: entry_label } // Set entry point
 }


pub fn cnst(dest: &str, val: i64) -> Instr { Instr::Const(dest.to_string(), val) }
pub fn branch(cond: &str, t: &str, f: &str) -> Instr { Instr::BranchIf(cond.to_string(), t.to_string(), f.to_string()) }
pub fn jump(target: &str) -> Instr { Instr::Jump(target.to_string()) }
pub fn ret() -> Instr { Instr::Ret{value: None} }
pub fn ret_val(val: &str) -> Instr { Instr::Ret { value: Some(val.to_string()) } } // Added Ret with value helper
pub fn print(s: &str) -> Instr { Instr::Print(s.to_string()) }
pub fn assign(d: &str, s: &str) -> Instr { Instr::Assign(d.to_string(), s.to_string()) }
pub fn binop(dest: &str, l: &str, op: Op, r: &str) -> Instr { Instr::BinOp(dest.to_string(), l.to_string(), op, r.to_string()) }
pub fn cmp(dest: &str, l: &str, op: CmpOp, r: &str) -> Instr { Instr::Cmp(dest.to_string(), l.to_string(), op, r.to_string()) }
pub fn phi(dest: &str, preds: Vec<(&str, &str)>) -> Instr {
    Instr::Phi(
        dest.to_string(),
        preds.into_iter().map(|(l, v)| (l.to_string(), v.to_string())).collect()
    )
}
pub fn func_param(dest: &str, index: usize) -> Instr {
    Instr::FuncParam { name: dest.to_string(), index }
}
// Added Call helper
